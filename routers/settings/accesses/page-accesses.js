...
router.get("/pages/:employee_memberID", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC StatisticAPI.GetMemberPageAccesses ${MemberID}, ${req.params.employee_memberID}`
  );
...
router.post("/", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC StatisticAPI.SaveChangedAccesses ${MemberID}, ${employee_memberID}, N'${JSON.stringify(
      records
    )}'`
  );
